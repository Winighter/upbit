...
class Upbit:
    ...
    def get_balance(self):
        ...
        position = 20 # 10 ~ 20 예수금 기준 투자할 금액 비율 (%)
    ...
    def min_candle_chart(self, _symbol, _minutes):
        ...
        ### INDICATOR ###
        _scalp = Indicators.rsi_scalp(_open, _high, _low, _close, 1)
        ...
        # BUY
        if _symbol not in self.balance_dict.keys() and long and self.ADD_LOCK == 0:
        ...
        if _symbol in self.balance_dict.keys():
            ...
            balance_deposit = math.trunc(balance*buy_price)

            profit = round(((_close[0] - buy_price) / buy_price) * 100, 2)

            if exit_long:
                Message(f"[{_symbol}] Close Position ${profit}")
                self.order_symbol("SELL", _symbol, balance)

            elif long and buy_price > _close[0] and self.deposit > balance_deposit and self.ADD_LOCK == 0:

                Message(f'[{_symbol}] Add Entry Position')
                self.ADD_LOCK = 1
                self.order_symbol("BUY", _symbol, balance_deposit)

        if self.ADD_LOCK == 1 and long == False:
            self.ADD_LOCK = 0

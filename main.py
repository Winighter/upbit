...
class Upbit:
    ...
    def get_balance(self):
        ...
        bSymbol_list = list(self.balance_dict.keys())

        for i in bSymbol_list:
            if i not in symbol_list:
                self.balance_dict.pop(i)

        threading.Timer(1, self.get_balance).start()
    ...
    def min_candle_chart(self):
        ...
        if self._BUY == True and self.balance_dict != {}:
            self._BUY = False

        if self._SELL == True and self.balance_dict == {}:
            self._SELL = False

        # BUY
        if self._BUY == False and long_condition == True and self.symbol not in self.balance_dict.keys():
        ...
        # SELL
        if self._SELL == False and long_end == True and self.symbol in self.balance_dict.keys():
        ...
        threading.Timer(2, self.min_candle_chart).start()

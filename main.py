...
class Upbit:
    ...
    def get_balance(self):
        ...
        position = 97 # 10 ~ 20 전체자산 기준 투자할 금액 비율 (%)
    ...
    def order(self, _side:str, _symbol:str, _amount:float, _price:float):

        if self.ORDER_LOCK == False:
            
            # Buy Market Price
            if _side == "BUY":
                self.exchange.create_order(_symbol, 'market', 'buy', _amount, _price)
                self.buy_order_list.append(_symbol)

            # Sell Martket Price
            else:
                self.exchange.create_market_sell_order(symbol=_symbol, amount=_amount)
    ...
    def min_candle_chart(self):
        ...
        _ema50 = Indicator.ema(_close, 50, None)
        _ema200 = Indicator.ema(_close, 200, None)

        long_condition = _ema50[2] <= _ema200[2] and _ema50[1] > _ema200[1]
        long_end = _ema50[2] >= _ema200[2] and _ema50[1] < _ema200[1]

        if self._ORDER == False and long_condition:
            self._ORDER = True
            Message(f'[UPBIT] long_condition')

        if self._ORDER == True and long_end:
            self._ORDER = False
            Message(f"[UPBIT] long_end")

        # BUY
        if long_condition == True and self.symbol not in self.balance_dict.keys() and self.symbol not in self.buy_order_list:

            bid_price = self.get_hoka(self.symbol)
            amount = float(format(self.deposit/bid_price, '.8f'))

            self.order("BUY", self.symbol, amount, bid_price)

        # SELL
        if long_end == True and self.symbol in self.balance_dict.keys() and self.symbol not in self.sell_order_list:

            balance = self.balance_dict[self.symbol]['balance']

            self.order("SELL", self.symbol, balance, 0)
        ...
        if data["type"] == "myOrder":
            ...
            for s in data["assets"]:
                ...
                if currency != "KRW":
                    ...
                    self.deposit = math.trunc(s["balance"])

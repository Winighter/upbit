...
class Upbit:
    ...
    def order(self, _side:str, _symbol:str, _amount:float, _price:float, _id:str):
    ...
    def min_candle_chart(self):
        ...
        url = f"https://api.upbit.com/v1/candles/minutes/{self.min_time}" # n분차트 1, 3, 5, 10, 15, 30, 60, 240
        ...
        for i in data:
            ...
            last_time = str((i['candle_date_time_kst']))
            self.open_list.append(open)
            self.low_list.append(low)
            self.high_list.append(high)
            self.close_list.append(close)

        self.min_candle_chart2(last_time)

    def min_candle_chart2(self, _time):

        url = f"https://api.upbit.com/v1/candles/minutes/{self.min_time}" # n분차트 1, 3, 5, 10, 15, 30, 60, 240
        querystring = {"market":self.symbol,"count":"200", "to": f"{_time}+09:00"} # count = 필요한 봉 갯수
        data = requests.request("GET", url, params=querystring).json()
        ...
        self.min_candle_chart3(last_time)

    def min_candle_chart3(self, _time):
        ...
        self.min_candle_chart4(last_time)

    def min_candle_chart4(self, _time):
        ...
        _ema50 = Indicator.ema(self.close_list, 50, None)
        _ema200 = Indicator.ema(self.close_list, 200, None)
